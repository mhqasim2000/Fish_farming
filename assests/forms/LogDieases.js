...
export default function LogDiease({ navigation, route }) {
  ...
  const submit = async () => {
    if (!pond?.id) {
      Alert.alert("Mortality", "Missing pond.");
      return;
    }
    if (!stockId) {
      Alert.alert("Mortality", "Select the affected species / batch.");
      return;
    }
    const n = Number(deadCount || 0);
    if (!Number.isFinite(n) || n <= 0) {
      Alert.alert("Mortality", "Enter the number of dead fish (greater than zero).");
      return;
    }
    if (n > available) {
      Alert.alert("Mortality", `You only have ${available} fish in this batch.`);
      return;
    }
      ...
      <ScrollView contentContainerStyle={styles.body} showsVerticalScrollIndicator={false}>
        ...
        {batches.length === 0 ? (
          <Card>
            ...
            <Text style={styles.emptyText}>Stock fish first, then you can record mortality.</Text>
          </Card>
        ) : (
          <>
            <Text style={styles.label}>Affected species / batch</Text>
            <View style={styles.pickerWrap}>
              <Picker selectedValue={stockId} onValueChange={(v) => setStockId(String(v || ""))} style={styles.picker}>
                <Picker.Item label="Select batch" value="" />
                {batches.map((b) => (
                  <Picker.Item
                    key={String(b.id)}
                    label={`${b.species || b.SpeciesName} — ${Number(b.quantity ?? b.Quantity ?? 0).toLocaleString()} available`}
                    value={String(b.id)}
                  />
                ))}
              </Picker>
            ...
            {!!selected && (
              <Card style={styles.infoCard}>
                <View style={styles.infoRow}>
                  ...
                  <Text style={styles.infoTitle}>{selected.species || selected.SpeciesName}</Text>
                </View>
                <Text style={styles.availableLine}>
                  Available in pond: <Text style={styles.availableStrong}>{available.toLocaleString()} fish</Text>
                </Text>
              </Card>
            )}

            <Text style={styles.label}>Number of diease fish</Text>
            <TextInput
              value={deadCount}
            ...
            <PrimaryButton title={submitting ? "Saving…" : "Record mortality"} onPress={submit} disabled={submitting} />
          </>
        )}
      </ScrollView>
    </View>
